...
* Author nya Radya, Farid, M.hadi.firmansya, Nazwa dan AGUSZ PAKEZ
* Tambahin ya zhayank
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code AGUSZ PAKEZ'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}* yang betah yah sayang 😘😘`
				...
				teks = `Akhirnya beban group berkurang 1,huss husss🐦 @${num.split('@')[0]} nitip pelmen milkita satu karung yah umm 🏃🏃`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
			...
			switch(command) {
			                ...
			                const uangku = checkATMuser(sender)
                                ...
                                case 'magernulis':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        await costum(mager(prefix, botName, ownerName), text, tescuk, cr)
                                        break
                                case 'imagemakermenu':
                                        if (!isRegister) return reply(mess.only.daftarB)
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Tuh ownerku omm, jangan lu culik 🤧🤧',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Kirim perintah ${prefix}snowwrite teks1|teks2, contoh ${prefix}snowwrite APBOT|AGUSZ`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`textnya mna omm?`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
               ...
               client.updatePresence(from, Presence.composing) 
				...
				case 'semoji':
					if (args.length < 1) return reply('emojinya mana um?')
                                        if (!isRegister) return reply(mess.only.B)
					ranp = getRandom('.png')
					rano = getRandom('.webp')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'alay':
                    ...
                    client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih omm...'})
                    await limitAdd(sender)
                    break
                    case 'glitch':
                    gh = body.slice(7)
                    ...
                    client.sendMessage(from, hasil, image, {quoted: mek, caption: 'nih omm'})
                    await limitAdd(sender)
                    break
                     case 'leave':
                    if (!isGroup) return reply(mess.only.group)
           ...
           break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'mampod Lu kekick😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
			...
			case 'grouptutup':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'dah jadi tod'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
            ...
            break
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo2':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih logonya tod..'})
                                        await limitAdd(sender)
                                        break
                                case 'lionlogo':
                                        var gh = body.slice(9)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'mutgrass':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih gambarnya umm...'})
                                        await limitAdd(sender)
                                        break
                                case 'undergocean':
                                        var gh = body.slice(12)
                                ...
                                case '8bit':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}8bit AGUSZ|APBOT')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
				...
				case 'clone':
					...
					break
                                ...
                                case 'event':
                                        ...
                                        break
                                case 'daftar':
                                        if (isRegister) return  reply(`「 SUDAH TERDAFTAR 」 \n\nKamu udah terdaftar jadi partner kami!! umm🐦🐦`)
                                        ...
                                        if (isGroup) {
                                                ...
                                                console.log(color('[READY REGISTER]'), color(time, 'yellow'), 'NAMA:', color(namaUser, 'cyan'), 'UMUR:', color(umurUser, 'cyan'), 'No Seri:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
                                        } else {
                                                addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                                                await reply(ind.registered(namaUser, umurUser, serialUser, time, sender))
                                                addATM(sender)
                                                addLevelingId(sender)
                                                console.log(color('[READY REGISTER]'), color(time, 'yellow'), 'NAMA:', color(namaUser, 'cyan'), 'UMUR:', color(umurUser, 'cyan'), 'No seri:', color(serialUser, 'cyan'))
                                        }
                                        break
                                case 'mining':
                                        if (!isRegister) return reply(mess.only.daftarB)
